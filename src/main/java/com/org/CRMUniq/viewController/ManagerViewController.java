...
public class ManagerViewController {
	...
	public String mysmartView(Model model, HttpSession session) {
		...
		List<Long> SalesUsersId = managerservice.getByID(SessionUser.getEID()).getSalesUser().stream()
				.map(s -> s.getSUID()).collect(Collectors.toList());
	...
	private String leads(Model model, HttpSession session) {
		...
		List<Leads> leads = leadService.getAllLeads().stream().filter(l -> {
			if (l.getLeadOwnerRole() != null 
					&& l.getLeadOwnerRole().equalsIgnoreCase("Manager")
					&& l.getLeadOwnerId() == SessionUser.getEID() && !l.getLeadStatus().equalsIgnoreCase("rejected")
